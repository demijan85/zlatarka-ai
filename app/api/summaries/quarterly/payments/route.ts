...
export async function GET(request: Request) {
  try {
    ...
    const xml = buildPaymentsXml(filtered.map((row) => ({ ...row, totalAmount: row.totalPremium })), now, {
    ...
    return new NextResponse(new Blob([payload], { type: 'application/xml; charset=utf-16le' }), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${getQuarterlyPaymentExportFileName('payments', year, quarter, language)}"`,
      },
    });
  } catch (error) {
